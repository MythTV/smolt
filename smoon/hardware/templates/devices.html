        ...
        <div class="tabbertab"><h2>Raw Devices</h2>
            <table id="stats" width="100%" border="0" cellpadding="3" cellspacing="3">
                <tr py:for='dev in devices["totalList"]'>
                    ...
                    <td nowrap="true"><strong>${'%.1f' % (float(dev.count) / devices["total"] * 100) } %</strong></td>
                    <td nowrap="true"><img py:for='i in range(1, int( float(dev.count) / devices["total"] * 1000 ))' src='/static/images/tile.png' /></td>
                </tr>
            </table>
        </div>
